                    ...
                    <div class="modal-body">
                        <form name="myForm" id="form1" novalidate>
                            <div class="form-group" ng-class="{ 'has-error' : !myForm.name.$pristine && myForm.name.$invalid }">
                                <label for="friend_link-name" class="control-label">名称:</label>
                                <input type="text" class="form-control" name="name" id="friend_link-name" ng-maxlength="15"
                                       required ng-model="friend_link.name">
                            ...
                            <div class="form-group" ng-class="{ 'has-error' : !myForm.url.$pristine && myForm.url.$invalid }">
                                <label for="friend_link-url" class="control-label">URL:</label>
                                <input type="url" class="form-control" name="url" id="friend_link-url" required ng-model="friend_link.url">
                            ...
                            <div class="form-group">
                                <label for="friend_link-image" class="control-label">头图<em class="text-danger">（必填）</em> :</label>
                                <input type="file" class="file-loading" id="file0" name="image">
                                <div id="errorBlock" class="help-block"></div>
                                <a ng-href="@{{imagesrc}}" data-lightbox="example-set" data-title="上传的图片" ng-show="upload_image"/>
                                    <img ng-src="@{{imagesrc}}" alt="已上传的图片" width="50px" height="50px">
                                </a>
                                <input type="hidden" class="form-control" id="File" ng-model="friend_link.image" required>
                            </div>

                            <div class="form-group" ng-class="{ 'has-error' : !myForm.desc.$pristine && myForm.desc.$invalid }">
                                <label for="friend_link-desc" class="control-label">网站描述:</label>
                                <input type="text" class="form-control" name="desc" id="friend_link-desc" required ng-model="friend_link.desc">
                            ...
                            <div class="form-group" ng-class="{ 'has-error' : !myForm.weight.$pristine && myForm.weight.$invalid }">
                                <label for="friend_link-weight" class="control-label">权重:</label>
                                <input type="number" class="form-control" name="weight" id="friend_link-weight" min="0" max="9999"
                                       required ng-model="friend_link.weight">
                            ...
                            <div class="form-group" ng-class="{ 'has-error' : !myForm.info.$pristine && myForm.info.$invalid }">
                                <label for="friend_link-info" class="control-label">备注:</label>
                                <input type="text" class="form-control" name="info" id="friend_link-info" required ng-model="friend_link.info">
    ...
    <script>
        $(function () {
            ...
            $('#file0').fileinput({
                ...
                //showRemove: false,
                //showUpload: false,
                uploadUrl: "{{route('api.image.upload', 'friendlink')}}", // server upload action
                //uploadUrl: "/admin/common/upload-image/friendlink", // server upload action
                uploadAsync: true,
                ...
                scope.friend_link.image = data.response.res;
        ...
        angular.module('myModule', ['bsTable'])
            .controller('myController', function ($scope, $http) {
                ...
                $scope.toggle = function (modalstate, id) {
                    ...
                    switch (modalstate) {
                        ...
                        case 'edit':
                            ...
                            $scope.id = id;
                            $http.get('/admin/friend-link/' + id)
                                .then(function successCallback(response) {
                                    $scope.friend_link = response.data;
                                    $scope.imagesrc = '/uploads/' +  $scope.friend_link.image;
                                    $scope.upload_image = true;
                                });
                            break;
                        default:
                ...
                $scope.save = function (modalstate, id) {
                    var url = "{{route('friend-link.store')}}";
                    var dataparam = $.param($scope.friend_link);
                    if (modalstate === 'edit') {
                        url += '/' + id;
